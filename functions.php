...
function getReservationsByUserId($utilisateur_id)
{
    ...
    $sql = "SELECT reservation.id, reservation.date_debut, reservation.date_fin, 
...
// function insertReservation($utilisateur_id, $vehicule_id, $date_debut, $date_fin)
// {
//     global $conn;
...
//               VALUES (?, ?, ?, ?)";
//     $stmt = $conn->prepare($query);
//     $stmt->bind_param("iiss", $utilisateur_id, $vehicule_id, $date_debut, $date_fin);
...
function checkExistingReservation($utilisateur_id, $vehicule_id, $date_debut, $date_fin)
{
    global $conn;

    $query = "SELECT COUNT(*) FROM reservation WHERE utilisateur_id = ? AND vehicule_id = ? AND date_debut = ? AND date_fin = ?";
    $stmt = $conn->prepare($query);
    $stmt->bind_param("iiss", $utilisateur_id, $vehicule_id, $date_debut, $date_fin);
    $stmt->execute();
    $count = 0;
...
function checkReservationDuration($date_debut, $date_fin)
{
    $date_debut_obj = new DateTime($date_debut);
    $date_fin_obj = new DateTime($date_fin);

    return $date_fin_obj->getTimestamp() - $date_debut_obj->getTimestamp() >= 3600;
}

function checkReservation($conn, $service_id, $vehicule_id, $date_debut, $date_fin)
{
    $sql = "SELECT * FROM reservation WHERE service_id = ? AND vehicule_id = ? AND ((date_debut <= ? AND date_fin > ?) OR (date_debut < ? AND date_fin >= ?) OR (date_debut >= ? AND date_fin <= ?))";
    $stmt = $conn->prepare($sql);
    $stmt->bind_param("iissssss", $service_id, $vehicule_id, $date_debut, $date_debut, $date_fin, $date_fin, $date_debut, $date_fin);
    $stmt->execute();
    $result = $stmt->get_result();
    $stmt->close();
...
{
    // Charger la configuration du serveur de messagerie
