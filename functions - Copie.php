...
// function check_reservation($conn, $service_id, $vehicule_id, $date_debut, $date_fin, $utilisateur_id) {
//     // Ajoutez utilisateur_id à la requête SQL
//     $sql_check = "SELECT * FROM reservation WHERE service_id = ? AND vehicule_id = ? AND utilisateur_id = ? AND ((date_debut >= ? AND date_debut < ?) OR (date_fin > ? AND date_fin <= ?))";
//     $stmt_check = $conn->prepare($sql_check);
//     // Ajoutez l'utilisateur_id au bind_param
//     $stmt_check->bind_param("iiissss", $service_id, $vehicule_id, $utilisateur_id, $date_debut, $date_fin, $date_debut, $date_fin);
//     $stmt_check->execute();
...
function sendEmail($userEmail, $subject, $message)
{
    ...
    try {
        ...
        // Destinataires
        $mail->setFrom('XXXXXXXXXX', 'Mailer');
