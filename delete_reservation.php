...
$stmt = $mysqli->prepare("SELECT reservation.*, service.nom as service_nom, vehicule.marque as vehicule_marque, vehicule.modele as vehicule_modele, vehicule.immatriculation as vehicule_immatriculation FROM reservation 
    INNER JOIN service ON reservation.service_id = service.id 
    INNER JOIN vehicule ON reservation.vehicule_id = vehicule.id 
    WHERE reservation.id = ? AND reservation.utilisateur_id = ?");
...
if ($reservation) {
    ...
    $message = "Votre réservation a été supprimée. Voici les détails de la réservation supprimée : Service: {$reservation['service_nom']}, Véhicule: {$reservation['vehicule_marque']} {$reservation['vehicule_modele']} - {$reservation['vehicule_immatriculation']}, Date de début: $date_debut, Date de fin: $date_fin.";
