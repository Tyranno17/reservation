...
if (isset($_POST['submit']) && isset($_POST['service_id']) && isset($_POST['vehicule_id']) && isset($_POST['date_debut']) && isset($_POST['date_fin']) && isset($_POST['destination'])) {
    ...
    if (!isset($errors)) {
        ...
        if (!checkExistingReservation($utilisateur_id, $vehicule_id, $date_debut, $date_fin, $destination)) {
            // Vérifier la disponibilité du véhicule pour les dates choisies
            if (isVehicleAvailable($vehicule_id, $date_debut, $date_fin, $utilisateur_id)) {
                // Si le véhicule est disponible, insérer la réservation dans la base de données
                if (insertReservation($utilisateur_id, $vehicule_id, $date_debut, $date_fin, $destination)) {
